...
public class TestUnAllocateUnitUsingRightClick extends BaseTest {
	...
	public void toUnAllocate() throws Exception{
		TestUnAllocateRightClickPage testUnAllocateRightClickPage = new TestUnAllocateRightClickPage(driver,configurationReader,browserName);
		boolean status = testUnAllocateRightClickPage.toUnAllocate();
		handleAssert(status);
	}
	
	@Test(dependsOnMethods={"toUnAllocate"})
